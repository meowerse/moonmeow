...
public class CursorFollowControllerTest {
    ...
    @Test
    public void aPanTheViewRefusesEndsTheFollowInsteadOfRequestingFramesForever() {
        // An overlay (the PC keyboard, bottomObstruction) covers the bottom of the stream: the
        // cursor sits at the desktop's last rows, which the visible rectangle can never reach
        // because the view is already panned as far as it goes. The follower must settle.
        CursorFollowController stuck = new CursorFollowController(view,
                new InlinePinchZoomController.ZoomTarget() {
                    @Override public void pinchBy(float s, float fx, float fy) { }
                    @Override public void panBy(float dx, float dy) { }   // clamped: no move
                    @Override public float getScaleFactor() { return 4f; }
                    @Override public float getChildX() { return -view.x * 4f; }
                    @Override public float getChildY() { return -view.y * 4f; }
                }, true, frames, new FollowLog(line -> { }));
        stuck.onStreamStarted(1920, 1080);
        stuck.onCursorPosition(960, 1075, true, 1);
        frames.runUi();
        assertTrue("frames stop once the view will not move", frames.settle() < 600);
        stuck.onStreamStopped();
    }
