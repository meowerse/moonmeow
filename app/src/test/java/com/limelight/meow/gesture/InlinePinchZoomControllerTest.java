...
public class InlinePinchZoomControllerTest {
    ...
    /**
     * Scoped to the controller: once ZOOM is latched a third finger IS swallowed here, and
     * that is correct. What stops it stealing the 3/4/5 finger gestures is that
     * {@code Game} offers every {@code pointerCount > 2} event to
     * {@code handleMultiTouchGesture} before this controller ever sees it — pinned by
     * {@link InlinePinchZoomDispatchOrderTest}, which is the only place that guarantee can
     * be stated.
     */
    @Test
    public void aThirdFingerAfterAZoomHasLatchedIsSwallowedHere() {
        down(100f, 100f);
        secondDown(100f, 100f, 200f, 100f);
        assertTrue(move(50f, 100f, 250f, 100f));
        assertTrue(controller.isZooming());
    ...
    public void resetDropsALatchedZoomWithoutFiringTheEndCallback() {
        ...
        assertFalse(controller.handle(MotionEvent.ACTION_MOVE, 2, 10f, 100f, 400f, 100f));
    }
