...
| 20 | Host cursor hidden (video, game) | any | Z | H | not chased while hidden, however much the mouse moves; the one exception is row 44 | `CursorFollowBindingTest.aHiddenCursorIsNotFollowed`, `CursorFollowControllerTest.aCursorAGameHidLongAgoIsNotChasedThoughTheMouseMoves` |
...
| 44 | The host reports its cursor hidden (0x3004 visible=false) | any | Z | host | a cursor that was visible and goes hidden while the user drives it (pointer input within 500 ms of the report; the emulator host hid it at the desktop edge mid-swipe) is still followed, decided per report so a follow in progress runs to its end; a cursor already hidden is not (row 20) | `CursorFollowControllerTest.aCursorTheHostCallsHiddenIsFollowedWhileTheUserMovesIt`, `…aFollowStartedOnADrivenHiddenCursorRunsToItsEnd`, `…aCursorHiddenByTheHostOnItsOwnIsNotChased`, `GameCursorFollowModesTest.aReportingHostThatHidesTheCursorAtTheEdgeIsFollowedToIt` |
